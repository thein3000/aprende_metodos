...
def metodo_lineal_con_funcion(x,y,ecuacion):
    lista_x = np.array(x)
    lista_y = np.array(y)
    lista_coeficientes = []
...
def generar_matriz(list_x,list_y,ecuacion1):
    ...
    for row in range(0,amount_of_rows): #For all rows in matrix
        ...
        row_buffer=[]
        for col in range(0,4): #For all but the last column
            if row == 0 and col == 0:
                row_buffer.append(n)
...
def num_at_nth_power_times_y_or_x(list_x,list_y,ecuacion,j):
        ...
        if j == 2:
            ...
            for numero in list_x:
                sum = sum + ecuacionmetodo(ecuacion,numero) * numero
        return sum
...
# print(metodo_lineal_con_funcion(x0,y0,t))
